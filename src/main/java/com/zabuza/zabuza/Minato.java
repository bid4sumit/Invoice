...
public class Minato {
	...
	public Minato() {
		...
		btnSubmit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
				invoice.setFrom(Util.dateFormat(fromdate.getDate()));
				invoice.setTo(Util.dateFormat(dateChooserto.getDate()));
				b=true;
				}catch(Exception n)
				{
					JOptionPane.showMessageDialog(frame, "Please Set From/To date");
					b=false;
				}
				...
				try {
					if(b==true && invoice.getInvoiceNo().length()>1){
						CreateInvoice.createInvoice(invoice);
						JOptionPane.showMessageDialog(frame, " invoice created "+invoice.getInvoiceNo()+".docx");
					}
					else {
						if(b==true)
						JOptionPane.showMessageDialog(frame, "Invoice Number missing");
					}
				} catch (Exception e1) {
					// TODO Auto-generated catch block
					JOptionPane.showMessageDialog(frame, "Unable to create Invoice File");
					e1.printStackTrace();
				}
	...
	private void calculate() {
		...
		temptotal = Util.round(temptotal*100)/100;		
		invoice.setTotal(temptotal);
		
		double tempcgst=(9*temptotal)/100;
		tempcgst = Util.round(tempcgst*100)/100;
		
		invoice.setSgst(tempcgst);
		invoice.setCgst(tempcgst);
		
		double gd = 2*tempcgst + temptotal;
		invoice.setGrandTotal(gd);
						
		sgst.setText(Double.toString(tempcgst));
		cgst.setText(Double.toString(tempcgst));
		total.setText(Double.toString(temptotal));
		gtotal.setText(Double.toString(gd));	
	...
	private void initialize() {
		...
		lblZabuza = new JLabel("Avval Courier & Logistics ");
		lblZabuza.setFont(new Font("Tahoma", Font.BOLD, 18));
		lblZabuza.setBounds(365, 13, 304, 16);
		...
		/*
		 * if(month<10){
		 * invoideDateText.setText(date.getDate()+"-0"+(date.getMonth()+1)+"-"+(date.
		 * getYear()+1900)); } else {
		 * invoideDateText.setText(date.getDate()+"-"+(date.getMonth()+1)+"-"+(date.
		 * getYear()+1900)); }
		 */
		invoideDateText.setText(Util.dateFormat(date));
		...
		billperiodfrom.setEnabled(false);
		billperiodfrom.setBounds(234, 78, 9, 22);
		//panel_2.add(billperiodfrom);
		//billperiodfrom.setColumns(10);
		
		billperiodto = new JTextField();
		billperiodto.setEnabled(false);
		billperiodto.setBounds(234, 113, 9, 22);
		...
		fromdate.setBounds(221, 78, 180, 22);
		panel_2.add(fromdate);
		
		dateChooserto = new JDateChooser();
		dateChooserto.setDateFormatString("dd-MM-yyyy");
		dateChooserto.setBounds(221, 113, 180, 22);
